...
export default function PayrollSettings() {
  ...
  useEffect(() => {
    if (settingsData) {
      form.reset({
          insuranceRate: settingsData.insuranceRate,
          // Make sure 'to' is a number, not Infinity
  ...
  const onSubmit = (data: PayrollSettingsFormData) => {
    if (!firestore) return;
    // Convert 'to' values of 0 back to Infinity for storage
    const dataToSave = {
        ...data,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-2xl">
        ...
        <div className="space-y-4">
            <FormLabel>پلکان‌های مالیاتی</FormLabel>
                ...
                <div key={field.id} className="grid grid-cols-1 md:grid-cols-7 gap-2 items-center p-2 border rounded-md">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="col-span-2">
                                <FormLabel className="text-xs">از (تومان)</FormLabel>
                    ...
                    />
                     <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="col-span-2">
                                <FormLabel className="text-xs">تا (تومان)</FormLabel>
            ...
            <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => append({ from: 0, to: 0, rate: 0 })}
            >
